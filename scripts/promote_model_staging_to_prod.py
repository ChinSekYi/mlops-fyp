"""
Script: promote_model_staging_to_prod.py

Promotes the champion model from the staging MLflow server to the prod MLflow server.
Loads environment variables from ENV_FILE (recommended: env/.env.stag_machine).

REMINDER:
- Only use this script in the mlops-stag EC2 machine (not on dev or prod EC2).
- Both mlflow-server-staging and mlflow-server-prod must be running and accessible.
...
print(f"Champion staging model is version {model_version}")
...
with mlflow.start_run(run_name="model_promotion") as run:
    mlflow.sklearn.log_model(
        sk_model=staging_champion_model,
        name=model_name_tag,
